...
public static class AgendaItemEndpoints
{
    public static void MapAgendaItemEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (CreateAgendaItemRequest request, AgendaItemService agendaItemService) =>
        {
        ...
        group.MapGet("/meeting/{meetingId}", async (Guid meetingId, AgendaItemService agendaItemService) =>
        {
            var items = await agendaItemService.GetByMeetingIdAsync(meetingId);
            return Results.Ok(items);
        });

        group.MapGet("/{id}", async (Guid id, AgendaItemService agendaItemService) =>
        {
        ...
        group.MapDelete("/{id}", async (Guid id, AgendaItemService agendaItemService) =>
        {
