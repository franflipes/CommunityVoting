...
public static class MeetingEndpoints
{
    public static void MapMeetingEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (CreateMeetingRequest request, MeetingService meetingService) =>
        {
        ...
        group.MapGet("/community/{communityId}", async (Guid communityId, MeetingService meetingService) =>
        {
            var meetings = await meetingService.GetMeetingsByCommunityAsync(communityId);
            return Results.Ok(meetings);
        }).RequireAuthorization();

        group.MapGet("/{id}", async (Guid id, MeetingService meetingService) =>
        {
        ...
        group.MapGet("/{id}/voting-eligible-data", async (Guid id, MeetingService meetingService) =>
        {
        ...
        group.MapGet("/{id}/quorum-status", async (Guid id, MeetingService meetingService) =>
        {
        ...
        group.MapPost("/{id}/attendance", async (Guid id, RecordAttendanceRequest request, MeetingService meetingService) =>
        {
        ...
        group.MapGet("/{id}/voting-settings", async (Guid id, MeetingService meetingService) =>
        {
        ...
        group.MapPut("/{id}/voting-settings", async (Guid id, UpdateVotingSettingsRequest request, MeetingService meetingService) =>
        {
        ...
        group.MapPut("/{id}", async (Guid id, UpdateMeetingRequest request, MeetingService meetingService) =>
        {
        ...
        group.MapGet("/{id}/participants", async (Guid id, MeetingService meetingService) =>
        {
            var participants = await meetingService.GetParticipantsAsync(id);
            return Results.Ok(participants);
        }).RequireAuthorization();

        group.MapDelete("/{id}", async (Guid id, MeetingService meetingService) =>
        {
